                      ...
                      <?php
                        foreach ($propietariosList as $row) {
                            ...
                            if ((json_decode($db->getPrivilegiosByRol($_SESSION['rol']['pkrol_id'],15))->action != "success") AND (json_decode($db->getPrivilegiosByRol($_SESSION['rol']['pkrol_id'],18))->action != "success")) {
                               echo "<td><button name='pkpro_id' value='$row->pkpro_id' type='submit' form='deleteForm' class='btn btn-danger btn-flat btn-block' disabled><i class='fa fa-trash'></i></button></td>";
                            } else {
